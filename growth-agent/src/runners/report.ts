...
export async function run({ weekOf, outputDir = DEFAULT_OUTPUT_DIR }: ReportOptions = {}): Promise<string> {
...
function resolveWeek(reference?: string): { start: string; end: string } {
...
function buildStub(weekStart: string, weekEnd: string): string {
