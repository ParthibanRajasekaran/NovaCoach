...
import { buildFrontMatter, renderMdx, toSlug } from '../lib/mdx.js';
const DEFAULT_OUT_DIR = path.resolve('out', 'content');
export async function run({ locale, query, brief, outDir = DEFAULT_OUT_DIR }) {
    if (!locale) {
...
async function loadBriefInsights(briefPath) {
    if (!briefPath) {
...
function buildOutline(query, summary) {
    return [
...
function buildJsonLd({ title, locale, canonical, metaDesc, }) {
    return {
...
function buildIntro(query, locale, summary) {
