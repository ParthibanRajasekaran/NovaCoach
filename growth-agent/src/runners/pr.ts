...
function buildRemoteUrl(repo: string, token: string): string {
  return `https://x-access-token:${token}@github.com/${repo}.git`;
}

function buildBranchName(locale: string): string {
  return `growth/${locale}-${new Date().toISOString().slice(0, 10).replaceAll('-', '')}`;
}

async function syncContent(srcRoot: string, destRoot: string, files: string[]): Promise<void> {
...
function buildPrBody(locale: string): string {
